...
import { getWalletBalanceEmailLogRunDetail } from '@/features/wallet-balance-email-logs/api/service.server';
...
export async function GET(_request: NextRequest, context: RouteContext) {
  ...
  try {
    const run = await getWalletBalanceEmailLogRunDetail(runId);
    if (!run) {
      return NextResponse.json(
        { success: false, message: '식대 잔액 이메일 run을 찾을 수 없습니다.' },
        { status: 404 }
      );
    }
