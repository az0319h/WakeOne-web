...
import { getContractExternalDocumentUrl } from '../contracts-table/columns';
import {
  CONTRACT_ATTACHMENT_STATUS_LABELS,
  CONTRACT_ATTACHMENT_STATUS_OPTIONS
} from '../contracts-table/options';
import { MyContractRowAction } from './row-action';

interface CreateMyContractColumnsOptions {
...
export function createMyContractColumns({
  onView
}: CreateMyContractColumnsOptions): ColumnDef<ContractDocument>[] {
  return [
    {
      id: 'document_number',
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className='block max-w-[220px] truncate'>{row.original.contract_target}</span>
      )
    },
    {
      id: 'contract_summary',
    ...
    {
      id: 'actions',
      cell: ({ row }) => <MyContractRowAction data={row.original} onView={onView} />
    }
  ];
}

export { getContractExternalDocumentUrl };
