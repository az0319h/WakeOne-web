...
- **타임스탬프**: **`@/lib/format-datetime` `formatAbsoluteDateTimeKo`만 사용** — 상대 시각·feature별 datetime 포맷·`date-fns` 직접 호출 **금지**
- **날짜만**: **`@/lib/format-date`** — `formatAbsoluteDateKo` · `formatAbsoluteDateKoOrPlaceholder` · `formatBirthdayDisplay` · `formatBirthdayMonthDay` — `toLocaleDateString`·feature별 `formatDate` **금지**
...
참조: `src/lib/format-datetime.ts` · `src/lib/format-date.ts` · `activity-logs-table/columns.tsx`
...
import {
  ...
  formatAbsoluteDateKoOrPlaceholder,
  formatBirthdayDisplay
} from '@/lib/format-date';

// 일반 날짜 → 2026-07-20
formatAbsoluteDateKo(contract.contract_start_date);

// null → '-'
formatAbsoluteDateKoOrPlaceholder(contract.approved_at);
