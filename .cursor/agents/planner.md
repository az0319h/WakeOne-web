...
- 타임스탬프 AC는 **절대 시각**(`formatAbsoluteDateTimeKo`, `yyyy-MM-dd (EEE) HH:mm:ss`) 기준 — 상대 시각 **금지**
- 날짜-only AC는 **`@/lib/format-date`** (`formatAbsoluteDateKo` `yyyy-MM-dd`, 생일은 `formatBirthdayDisplay`) 기준 — `toLocaleDateString` **금지**
