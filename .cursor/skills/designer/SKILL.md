...
plan에 **3 preview 게이트**가 있거나, 사용자에게 **구현 방향 선택**을 요청할 때는 **텍스트 설명만으로 끝내지 않는다**.

### 터미널 목업 (참조 스타일)

각 preview마다 **ASCII 터미널 와이어**를 1블록씩 출력한다. 참조: macOS 터미널 창 + `npm run preview` + 테두리 라벨 박스 UI.
...
**와이어 안에 넣을 요소 (해당 preview에 맞게):**

- `PageContainer` 제목·부제 한 줄
- 필터/툴바 (Combobox · Select · Search placeholder)
- **실제 한국어 샘플 데이터** 2~3행 (action 라벨·대상·결과 Badge)
- expand/카드/요약 카드 등 **선택안 차이**가 보이게
- shadcn 톤 힌트: `[ 본인 ▼ ]` · `성공` `실패` · `···` pagination

**각 preview 블록 끝에 1줄 요약:** 장점 · 단점 · 구현 난이도(낮/중/높).

**선택 게이트:** 사용자가 **`preview-1` / `preview-2` / `preview-3`** (또는 plan 지정 키워드) 중 **정확히 하나** 입력하기 전 **BE/FE 착수 금지**.

> `/root` 파이프라인: designer Step 4에서 3 preview 터미널 목업 출력 → 사용자 선택 → Step 5~6 및 FE/BE handoff.
...
**Designer gate가 있는 plan:** Step 4 **앞단**에서 위 §Preview 터미널 목업 3개를 먼저 출력하고, 사용자 선택 후에만 컴포넌트 트리 상세를 확정한다.
