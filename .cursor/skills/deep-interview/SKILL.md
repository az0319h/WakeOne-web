...
description: |
  ...
  **예외:** `/root`·planner Phase 1에서는 **항상** 실행한다 (아래 §`/root` 강제).
...
## `/root` · planner Phase 1 — **생략 금지 (최우선)**

`/root` 파이프라인 또는 planner `[planner Phase 1/4] deep-interview`에서 이 스킬을 호출할 때:

| 규칙 | 내용 |
|------|------|
| **적용** | 위 description의 「구체적이면 생략」**무시** |
| **최소** | 사용자가 요구를 길게 적었어도 **확인·제외·AC·권한** 질문 **최소 3문항** (한 번에 1문항 형식 유지 가능 — 3문항을 한 메시지에 나열 후 **답변 대기**) |
| **금지** | TBD만 남기고 인터뷰 생략 · plan 저장과 동시 종료 |
| **종료** | 사용자 답변이 채팅에 온 뒤에만 Phase 2(battle-plan) 진행 |

root·planner는 Phase 1 완료 전 `docs/plans/` **저장 금지**.
